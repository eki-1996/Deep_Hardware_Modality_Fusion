...
class Exposuref(nn.Module):
    ...
    def __init__(self, in_channel=16, out_channel=1, kernel_size=8, binarize_type='full', kwargs={}):
        ...
        self.weight = Parameter(torch.Tensor(self.out_channel, self.in_channel, self.kernel_size, self.kernel_size))

        self.reset_parameters()

    def reset_parameters(self):
        self.stdv = math.sqrt(1.5 / (self.kernel_size * self.kernel_size * self.in_channel))
    ...
    def forward(self, input):
        if self.training and self.noise_count > 0:
            ...
            contious_weight = (self.weight * (1-p) + 
                               ...
                               ).repeat(1, 1, int(input.shape[-2] / self.kernel_size), int(input.shape[-1] / self.kernel_size))
        else:
            contious_weight = self.weight.repeat(1, 1, int(input.shape[-2] / self.kernel_size), int(input.shape[-1] / self.kernel_size))
