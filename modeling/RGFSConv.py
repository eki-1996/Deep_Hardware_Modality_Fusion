...
class RGFSConv(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        on_gpu = True if x.is_cuda else False

        y = torch.zeros((inter_feature.size()[:3] + (self.output_channel,))).permute(0, 3, 1, 2)
        if on_gpu: y = y.cuda()
        for i in range(10):
            index = torch.zeros((x.size()[0], x.size()[2], x.size()[3], 1))
            if on_gpu: index = index.cuda()
            index[mask==i] = 1
