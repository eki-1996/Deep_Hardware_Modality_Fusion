...
class TensorboardSummary(object):
    ...
    def visualize_test_image(self, writer, dataset, image, label, output, fused_img, global_step):
        ...
        grid_image = make_grid(decode_seg_map_sequence(torch.squeeze(label, 1).detach().cpu().numpy(),
                                                       dataset=dataset), label.size(0), normalize=False, range=(0, 255))
        writer.add_image('Test Groundtruth label', grid_image, global_step)
        grid_image = make_grid(fused_img.clone().cpu().data, fused_img.size(0), normalize=True)
        writer.add_image('Test fused Image', grid_image, global_step)